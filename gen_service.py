...
async function sendMagicLink(slug, email, token) {
  ...
  await transporter.sendMail({
    from: process.env.SMTP_FROM || "gallery@yourdomain.com",
    to: email,
...
w("service/src/index.js", INDEX)

GALLERIES_EXAMPLE = """{
  "sample-client": {
    "dir": "REPLACE_WITH_ABSOLUTE_PATH/sample-client",
    "allowedEmails": ["client@example.com"]
  }
}
"""
w("service/data/galleries.example.json", GALLERIES_EXAMPLE)
...
SMTP_FROM=gallery@yourdomain.com
"""

w(".env.example", ENV_EXAMPLE)
w("service/.env.example", ENV_EXAMPLE)
